...
define(function(require, exports, module) {
	...
	var dialogUrl = "risk/usedtaboo/openDialog";
	var getLatestDateUrl = "monitor/hotlineinconversationreview/getLatestDate";
	var submitCheckUrl = "risk/usedtaboo/submitCheck";
	//连接配置
	var width='500px';
	var height='580px';
	...
	HotlineTaboo.prototype = {
		...
		init: function() {
			...
			var dgColumns = [[
			    ...
			    {field: 'ismute45s',title: '是否静音时长大于45秒',width: 150},
			    {field: 'muteLocation',title: '静音超时的语句位置',width: 150,
			    	formatter: function(val, row, index) {
		...
		initEleEvent:function(){
			...
			self.$toolbar.on("click",".bda-btn-start",function(){
				...
				//提交表单设置
				self.$grid.datagrid({
					url : 'risk/usedtaboo/getTabooJudgeDetail',
					queryParams:self.getFormData()
				});
				return false;
			})
...
//				}
				...
				$.ajax({
					url:"risk/usedtaboo/exportJudgeDetail",
					type:"post",
		...
		detailDialog:function(){
			...
			function setContactDetail() {
				$.ajax({
					url: "risk/usedtaboo/getSessionDetail",
					type: "post",
					data: {
