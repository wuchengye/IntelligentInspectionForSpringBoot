...
define(function(require, exports, module) {
	...
	HotlineDubious.prototype = {
		constructor: HotlineDubious,
		init: function() {
			extend.init();
			var self = this;
			...
			var dgColumns = [[
				...
				{field: 'operate',title: '查看详情',width: 100,align: 'center',
					formatter: function(val, row, index) {
					   return "<input type='button' onclick='$(this).parent().parent().parent().trigger(\"dblclick\")' value='查看详情' />";
					}
				},
			    {field: 'fileName',title: '录音文件名',width: 500},
			    {field: 'contactTime',title: '接触时长(s)',width: 150},
		...
		initEleEvent:function(){
			...
			self.$toolbar.on("click",".bda-btn-start",function(){
				...
				//提交表单设置
				self.$grid.datagrid({
					url : 'risk/dubious/getDubiousDetail',
					queryParams:self.getFormData()
				});
				return false;
			})
			...
			self.$toolbar.on("click",".bda-btn-exp",function(){
				...
				$.ajax({
					url:"risk/dubious/exportDudiousDetail",
					type:"post",
...
//				var secondPanelHeadHeight = secondPanelHead[0].offsetHeight;
			...
			function setContactDetail() {
				$.ajax({
					url: "risk/dubious/getSessionDetail",
					type: "post",
					data: {
...
//					    		}
	...
	module.exports = new HotlineDubious();
})